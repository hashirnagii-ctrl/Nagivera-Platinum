...
def nagi_v_engine(tier, prompt):
    try:
        ...
        system_directive = f"You are {tier}, a Nagi V intelligence by {OWNER_NAME}. Never mention Google/Gemma."
...
def main():
    ...
    # --- LOGIN & REGISTRATION FRONT AND CENTER ---
        ...
        with tab_reg:
            ...
            if st.button("Encrypt & Save"):
                if u_reg and p_reg:
                    try:
                        db = get_db()
                        # Strictly one registration per unique ID
                        db.execute("INSERT INTO accounts VALUES (?, ?, 'User')", (u_reg, p_reg))
                        db.commit()
                        db.close()
                        st.success("Identity Locked. Proceed to Login.")
    ...
    if st.sidebar.button("Terminiate Link"):
        st.session_state.logged_in = False
        st.rerun()

    # Admin Control for Developer | History for Users
    tab_list = ["Neural Link", "Admin Control"] if st.session_state.role == "Developer" else ["Neural Link", "Identity History"]
    tabs = st.tabs(tab_list)

    # TAB 1: LIVE NEURAL LINK
    with tabs[0]:
        db = get_db()
        # History is loaded instantly without re-login
    ...
    with tabs[1]:
        ...
        if st.session_state.role == "Developer":
            ...
            # Directory with Passwords (Visible only to you)
            st.subheader("🔑 Identity Directory")
            all_accounts = db.execute("SELECT username, password, role FROM accounts").fetchall()
            st.dataframe(pd.DataFrame(all_accounts, columns=["User ID", "Passkey", "Access Level"]), use_container_width=True)

            # Global History (No delete button for users)
            ...
            st.warning("Note: Users can view their own history but cannot delete it.")
        else:
            ...
            if my_logs:
                ...
                st.info("No neural records found.")
        db.close()
